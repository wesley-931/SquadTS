...
export type AdminsMap = Record<string, AdminPermissions>;

export default async function fetchAdminLists(
  ...
  for (const [idx, list] of adminLists.entries()) {
    ...
    try {
      switch (list.type) {
        ...
        case 'local': {
          const listPath = path.isAbsolute(list.source)
            ? list.source
            : path.resolve(process.cwd(), list.source);
        ...
        case 'ftp': {
          ...
          const [loginString, hostPathString] = list.source
            .substring('ftp://'.length)
            .split('@');
          const [user, password] = loginString
            .split(':')
            .map((v) => decodeURI(v));
          ...
          const remoteFilePath =
            pathStartIndex === -1
              ? '/'
              : hostPathString.substring(pathStartIndex);
          const [host, portStr = '21'] = hostPathString
            .substring(
              0,
              pathStartIndex === -1 ? hostPathString.length : pathStartIndex,
            )
            .split(':');
        ...
        default:
          throw new Error(
            `Unsupported AdminList type: ${(list as AdminListSource).type}`,
          );
      }
    } catch (error) {
      Logger.verbose(
  ...
  Logger.verbose(
    'SquadServer',
    1,
    `${Object.keys(admins).length} admins loaded...`,
  );
  return admins;
}
